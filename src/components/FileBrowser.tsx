...
export function FileBrowser({ friendId, location, online, onBack }: { friendId: string; location: SharedLocation; online: string; onBack?: () => void }) {
  ...
  const actionSheet = dialog && <div className="location-modal dialog-overlay"><form className={MOBILE_UI ? "dialog mobile-sheet" : "card dialog"} role="dialog" aria-modal="true" aria-labelledby="location-action-title" onSubmit={e => { e.preventDefault(); void run() }} onKeyDown={e => {
