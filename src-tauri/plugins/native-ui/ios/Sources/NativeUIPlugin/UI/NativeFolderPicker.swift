...
@MainActor enum NativePresentation {
    static func waitForPickerDismissal() async throws {
        ...
        for _ in 0..<100 {
            ...
            if !(controller is UIDocumentPickerViewController) && !(controller is PHPickerViewController) && !controller.isBeingDismissed { return }
...
@MainActor final class NativeFolderPicker: NSObject, UIDocumentPickerDelegate {
    ...
    func pick() async throws -> String? {
        guard continuation == nil else { throw failure("A folder picker is already open.") }
        ...
        guard !(presenter is UIDocumentPickerViewController), !(presenter is UIActivityViewController), !presenter.isBeingDismissed else { throw failure("Finish the current sheet first.") }
        let picker = UIDocumentPickerViewController(forOpeningContentTypes: [.folder], asCopy: false)
        picker.delegate = self; picker.allowsMultipleSelection = false; picker.isModalInPresentation = true
