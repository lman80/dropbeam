            ...
            None => {
                ...
                match ret {
                    ...
                    | Some(Err(PathError::MaxPathIdReached)) => {
                        ...
                        // DropBeam patch (n0-computer/iroh#4509): with K connections to one
                        // remote, every retry tick re-queued each failing address once per
                        // connection, so the queue grew geometrically (multi-GB, then an
                        // allocation failure / abort). Queue each address once, and bound it.
...
/// DropBeam patch (n0-computer/iroh#4509): upper bound on [`State::pending_open_paths`].
