...
struct WatchActionsArgs: Decodable {
  let handler: Channel
}

class NotificationPlugin: Plugin {
  private var actionChannel: Channel?
  private var pendingActions = [ReceivedNotification]()

  // A tap may arrive before the webview has installed its listener.
  func deliverAction(_ action: ReceivedNotification) {
    if let channel = actionChannel {
      try? channel.send(action)
    } else {
      pendingActions.append(action)
    }
    try? trigger("actionPerformed", data: action)
  }

  @objc public func watchActions(_ invoke: Invoke) throws {
    let args = try invoke.parseArgs(WatchActionsArgs.self)
    actionChannel = args.handler
    for action in pendingActions { try args.handler.send(action) }
