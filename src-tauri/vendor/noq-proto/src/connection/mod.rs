...
impl Connection {
    ...
    fn populate_packet<'a, 'b>(
        ...
        // DropBeam patch: ACK_FREQUENCY parameters to hand to every path's controller.
        let mut ack_frequency_update: Option<(u64, Duration)> = None;
            ...
            && mem::replace(&mut space.pending.ack_frequency, false)
        {
            ...
            ack_frequency_update = Some((config.ack_eliciting_threshold.into_inner(), max_ack_delay));
        }
        ...
        // DropBeam patch: the peer applies an ACK_FREQUENCY frame to every path it has
        // ("Update the params for all of our paths" in the frame handler), but only the
        // controller of the path that carried the frame was told. The others kept assuming an
        // ACK every other packet, so their BBR sized its minimum window (and offload budget)
        // for that while the peer actually held ACKs for `ack_eliciting_threshold` packets or
        // max_ack_delay. Tell every path.
        if let Some((threshold, max_ack_delay)) = ack_frequency_update {
            for (id, p) in self.paths.iter_mut() {
                if *id != path_id {
                    p.data
                        .congestion
                        .on_ack_frequency_update(threshold, max_ack_delay);
                }
            }
        }
    }
