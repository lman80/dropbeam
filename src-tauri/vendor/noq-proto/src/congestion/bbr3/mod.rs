...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
enum AckPhase {
    /// equivalent to ACKS_INIT: not probing bandwidth; not getting probe feedback.
    /// DropBeam patch: draft-06 state, see `adapt_long_term_model`.
    Init,
...
pub struct Bbr3 {
    ...
    /// DropBeam patch switch (see [`Bbr3Config::dropbeam_fixes`]).
    dropbeam_fixes: bool,
}
...
impl Bbr3 {
    fn new(config: Arc<Bbr3Config>, current_mtu: u16) -> Self {
        ...
        Self {
            ...
            dropbeam_fixes: config.dropbeam_fixes,
        }
    }
    ...
    fn adapt_long_term_model(&mut self) {
        ...
        if self.ack_phase == AckPhase::ProbeStopping && self.round_start {
            // DropBeam patch (draft-ietf-ccwg-bbr-06 section 5.3.3.6, and Linux
            // `bbr_adapt_upper_bounds`): the end of the probe feedback leaves ACKS_PROBE_STOPPING
            // for ACKS_INIT and clears `bw_probe_samples`. draft-05, which this code follows,
            // omitted both, so the flow stayed in ProbeStopping through all of DOWN and CRUISE
            // and advanced the max-bw filter on EVERY round start. `MAX_BW_FILTER_LEN` = 2 then
            // spans two round trips instead of two probe cycles: max_bw tracks the last ~2
            // rounds of delivery rate, decays by the 0.9 DOWN pacing gain every round, and any
            // Wi-Fi fade longer than ~2 RTTs wipes the model and the cwnd with it. And with
            // `bw_probe_samples` stuck true, a single random loss in DOWN/CRUISE (on a small
            // BDP one packet is > LOSS_THRESH of the flight) cuts `inflight_longterm`.
            if self.dropbeam_fixes {
    ...
    fn maybe_go_down(&mut self) -> bool {
        // DropBeam patch: `adapt_long_term_model` has already raised `inflight_longterm` for
        // this ACK (`probe_inflight_long_term_upward`), but `cwnd` only follows in `set_cwnd`
        // afterwards, so a flow whose cwnd IS pinned to inflight_longterm reads a few bytes
        // below it on every ACK that raised the bound, falls through to the full-bw plateau
        // check and leaves PROBE_UP after 3 rounds while still bound by inflight_longterm. With
        // the long-term model no longer decaying every round (see `adapt_long_term_model`),
        // that plateau is real and the flow rediscovers bandwidth one tiny step per cycle.
        // Treat "within one packet of the bound" as bound by it.
        let slack = if self.dropbeam_fixes { self.smss } else { 0 };
        if self.is_cwnd_limited && self.cwnd.saturating_add(slack) >= self.inflight_longterm {
            self.reset_full_bw();
            if let Some(rate_sample) = self.rs {
                self.full_bw = rate_sample.delivery_rate;
            }
...
impl Bbr3 {
    /// equivalent to BBR.MinPipeCwnd, "the minimal C.cwnd value BBR targets, to allow
    /// pipelining with endpoints that follow an 'ACK every other packet' delayed-ACK policy:
    /// 4 * C.SMSS", i.e. two ACKs' worth of packets.
    ///
    /// DropBeam patch: generalized to the ACK-frequency extension. With an ack-eliciting
    /// threshold of N the peer ACKs every N+1 packets, so two ACKs' worth is 2 * (N + 1)
    /// packets (4 for the RFC 9000 default N = 1, unchanged). Below N+1 packets in flight the
    /// peer never reaches its threshold and holds every ACK for max_ack_delay (25 ms): each
    /// round then takes RTT + 25 ms, the delivery rate the model measures is
    /// cwnd / (RTT + 25 ms), and on a LAN (min_rtt of a few ms) the BDP that rate implies is a
    /// fraction of the cwnd that produced it, so `cwnd_gain * BDP` (and the offload budget,
    /// which also scales with that depressed max_bw) never climbs back past the threshold.
    /// That fixed point is the "cwnd pinned at 5808/7166 bytes, 0.2 MB/s for minutes" trap.
    fn min_pipe_cwnd_for(&self, smss: u64) -> u64 {
        if !self.dropbeam_fixes {
            return 4 * smss;
        }
        let per_ack = self.ack_eliciting_threshold.saturating_add(1).min(64);
        Ord::max(4, 2 * per_ack) * smss
    }
    ...
    fn on_mtu_update(&mut self, new_mtu: u16) {
        ...
        self.min_pipe_cwnd = self.min_pipe_cwnd_for(self.smss);
        self.set_send_quantum();
        self.set_cwnd();
    }
...
impl Bbr3Config {
    /// DropBeam patch: enable the fixes for the "cwnd pinned at the floor" trap: the
    /// draft-06 end-of-probe ACKS_INIT transition (max-bw filter spans probe cycles, not
    /// rounds) and an ACK-frequency-aware MinPipeCwnd. Off by default (stock noq behavior).
    pub fn dropbeam_fixes(&mut self, value: bool) -> &mut Self {
        self.dropbeam_fixes = value;
        self
    }
